  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        ...
        <KpiCard label="Online %" value={`${((active / total) * 100).toFixed(1)}%`} delta={0.8} />
        <KpiCard label="Avg Battery" value="78%" delta={-1.2} />
        <KpiCard label="Avg Signal (RSSI)" value="-72 dBm" delta={2.1} />
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border bg-card p-5 shadow-card lg:col-span-2">
          ...
          <TrendArea data={readSuccessTrend} x="day" series={[{ key: "success", label: "Success %", color: "var(--color-success)" }, { key: "failed", label: "Failed %", color: "var(--color-critical)" }]} height={220} />
        </div>
        <div className="rounded-xl border bg-card p-5 shadow-card lg:col-span-3">
          ...
          <TrendLine data={realtimeIngestion} x="hour" series={[{ key: "reads", label: "Reads/hr", color: "var(--color-primary)" }]} height={220} />
        </div>
      </div>
    </>
  );
